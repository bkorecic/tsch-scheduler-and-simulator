...
uint32_t epsilon_n = MAB_EPSILON_N;
...
uint8_t fhssDistributedBlacklistMABBestArmChan(Node_t *parent, Node_t *child, uint64_t asn)
{
    ...
    /* Every MAB_EPSILON_TIMESLOTS_TO_DIVIDE time slots we divide Epsilon by two */
    if (((asn + 1) % MAB_EPSILON_TS_INCR_N) == 0)
    {
        epsilon_n++;
    }
   ...
   /* Every MAB_EPSILON_TIMESLOTS_TO_DIVIDE time slots we divide Epsilon by two */
    ...
    else
    {
        ...
        for (ListElem *elem = ListFirst(&parent->channels); elem != NULL; elem = ListNext(&parent->channels, elem))
        {
            ...
            freq = fhssOpenwsnChan(freq_off, asn);
            if (parent->avg_reward[child->id][freq] > THRESHOLD_REWARD_GOOD_ARM)
            {
                return (freq);
            }
        }
